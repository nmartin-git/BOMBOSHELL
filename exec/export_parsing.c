...
/*   Updated: 2025/04/26 19:22:10 by nmartin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_input	*ignore_redir(t_input *arg_lst)
{
	while (arg_lst->next && arg_lst->next->token != PIPE
		...
		&& (arg_lst->next->token == INFILE
			|| arg_lst->next->token == HERE_DOC
			|| arg_lst->next->token == APPEND
			|| arg_lst->next->token == OUTFILE))
	{
...
int	export_parsing_utils(t_input *arg_lst, t_input *first)
{
	...
	{
		arg_lst->arg = ft_strjoin_free(arg_lst->arg, arg_lst->next->arg);
		del = arg_lst->next;
	...
	if (arg_lst->next->token == WORD_S_QUOTE
		...
		quotes = 0;
	first->arg = ft_strjoin_free (first->arg, arg_lst->next->arg);
	del = arg_lst->next;
	arg_lst->next = arg_lst->next->next;
	free(del);
	return (quotes);
...
void	export_quotes(t_input *arg_lst, int i)
{
	...
	if (arg_lst->next->token == SPACES)
		...
		str = ft_strjoin_free(ft_strdup(&arg_lst->arg[i]),
				ft_strdup(arg_lst->next->arg));
	arg_lst->arg[i] = '\0';
	str = ft_str_quotes(str, '"', '"');
	del = arg_lst->next;
	arg_lst->next = arg_lst->next->next;
	free(del);
...
void	export_get_arg(t_input *arg_lst, int i, int quotes)
{
	...
	if (quotes && arg_lst->arg[i] && arg_lst->next)
		...
		arg_lst->arg = ft_strjoin_free(arg_lst->arg, ft_strdup("\"\""));
	else if (!arg_lst->arg[i]
		&& arg_lst->next->token != PIPE && arg_lst->next->token != BOOL)
	{
		arg_lst->next->arg = ft_str_quotes(arg_lst->next->arg, '"', '"');
		arg_lst->arg = ft_strjoin_free (arg_lst->arg, arg_lst->next->arg);
		del = arg_lst->next;
		arg_lst->next = arg_lst->next->next;
		free(del);
	...
	{
		str = ft_str_quotes(ft_strdup(&arg_lst->arg[i]), '"', '"');
		arg_lst->arg[i] = '\0';
		arg_lst->arg = ft_strjoin_free(arg_lst->arg, str);
	}
}
